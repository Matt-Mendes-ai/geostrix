  ...
  return (
    <div style={overlay} onClick={onClose}>
      <div style={panel} role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
        <div style={header}>
          <div>
            <div style={{ fontSize: 15, color: "#8a6a1f", fontWeight: 600 }}>SQL workspace</div>
            <div style={{ fontSize: 11, color: "#94a1b0", marginTop: 2 }}>Ad hoc SQL against whatever's currently loaded — a snapshot taken when this opened, not a live connection.</div>
          </div>
        ...
        <div style={{ padding: 16, overflow: "auto", display: "flex", gap: 16, flex: 1, minHeight: 0 }}>
          <div style={{ width: 200, flexShrink: 0, overflow: "auto" }}>
            ...
            {state.status === "loading" && <div style={{ fontSize: 12, color: "#55606e" }}>Building…</div>}
            {state.status === "ready" && state.tables.length === 0 && <div style={{ fontSize: 11.5, color: "#94a1b0" }}>Nothing loaded yet — import collars/assays/layers first.</div>}
            {state.status === "ready" && state.tables.map((t) => (
              <div key={t.name} style={{ marginBottom: 8, fontSize: 11 }}>
                <div style={{ fontWeight: 600, color: "#1a2028" }}>{t.name}</div>
                <div style={{ color: "#94a1b0", lineHeight: 1.5 }}>{t.columns.join(", ")}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8, minWidth: 0 }}>
            {state.status === "error" && (
              <div style={{ padding: "8px 10px", background: "#2a1f1f", border: "1px solid #4a2f2f", borderRadius: 6, fontSize: 11.5, color: "#e0a0a0" }}>Couldn't set up the SQL engine: {state.message}</div>
            )}
            {emptyLoaded && state.status === "ready" && (
              <div style={{ fontSize: 11.5, color: "#94a1b0" }}>No data loaded yet — import collars, assays, or a layer first, then reopen this workspace.</div>
            )}
            <textarea
              value={sql}
              ...
              spellCheck={false}
              style={{ width: "100%", height: 100, fontFamily: "monospace", fontSize: 12.5, padding: 10, border: "1px solid #d9dce1", borderRadius: 6, resize: "vertical", color: "#1a2028" }}
            />
            ...
            {queryError && (
              <div style={{ padding: "8px 10px", background: "#2a1f1f", border: "1px solid #4a2f2f", borderRadius: 6, fontSize: 11.5, color: "#e0a0a0" }}>{queryError}</div>
            )}
            {result && (
              <div style={{ overflow: "auto", flex: 1, minHeight: 0 }}>
                <div style={{ fontSize: 11, color: "#94a1b0", marginBottom: 4 }}>{result.rows.length} row{result.rows.length === 1 ? "" : "s"}</div>
                ...
                {result.rows.length > 500 && <div style={{ fontSize: 10.5, color: "#94a1b0", marginTop: 4 }}>Showing the first 500 of {result.rows.length} rows — export to CSV for the full result.</div>}
              </div>
            )}
          </div>
        </div>
...
const th = { padding: "4px 8px", color: "#55606e", fontWeight: 500, textAlign: "left", borderBottom: "1px solid #d9dce1", position: "sticky", top: 0, background: "#ffffff" };
